...
const NewTransactionScreen = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    if (!validate()) {
      toast.warning('Champs invalides', 'Vérifiez les informations saisies');
      return;
    }
    setLoading(true);
    try {
      ...
      toast.error('Échec de la transaction', msg);
    } finally {
      setLoading(false);
    }
  };
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 24 }}>
            <TouchableOpacity
              ...
              style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: theme.surface, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}
            >
          ...
          {/* Error banner */}
          {errors.general && (
            <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: theme.colors.errorLight, borderRadius: 12, padding: 12, marginBottom: 16, gap: 8 }}>
          ...
          <Button title={t('transactions.confirmButton')} onPress={handleSubmit} loading={loading} fullWidth size="lg" style={{ marginTop: 8 }} />
        </ScrollView>
